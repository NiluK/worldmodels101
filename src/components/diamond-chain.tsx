...
const TEXT: LocaleText<Text> = {
  ...
  zh: {
    ...
    v0: "十二个环节，每一个都得撑住。",
    vBroke: (m, left) =>
      `这一次停在${m}。还差 ${left} 个环节，而这条链不给部分分。`,
    vWon: (x) => `这一次拿到了钻石。按这个概率，大约每十次里会有 ${x} 次。`,
    vRare: "按这个概率，钻石只是个罕见的意外，所以没有人是靠运气走到那里的。",
    aria: (state) =>
      `一条十二个 Minecraft 里程碑组成的链，终点是钻石。${state}`,
    ariaFresh: "还没跑过，所以每个环节都是空的。",
    ariaBroke: (m) => `这一次断在${m}。`,
    ariaWon: "这一次走到了钻石。",
  },
};
