...
const TEXT: Record<Locale, Text> = {
  ...
  zh: {
    ...
    predicted: "预测",
    actual: "实际",
    frameLine: "损失 0 = 这一帧被预测出来了",
    learned: "学到了东西",
    collapsed: "塌缩了",
    lossZero: "损失 0",
    left: "左转",
    right: "右转",
    sameCurve: "两种情况下曲线看起来一模一样",
    probeYes: "分得出左右：能",
    probeNo: "分得出左右：不能",
    illustrative: "曲线为示意",
    btnPixel: "揭示像素那一组",
    ...
    reset: "重来",
    v0: "两条曲线都降到了零。按「揭示」。",
    v1: "像素损失为零只意味着一件事：这一帧被预测出来了。",
    v2: "嵌入损失为零可能意味着两件事，而曲线分不出是哪一件。",
    v3: "只有测试嵌入能做什么，才能把两者分开。",
    rLossPixel: "最终的像素损失",
    rLossEmbed: "最终的嵌入损失",
    rTells: "这个零告诉你什么",
    everything: "一切",
    nothing: "单凭它，什么也说明不了",
    notYet: "尚未揭示",
    ariaPixel: "像素损失随训练降到零。揭示后：一帧预测与一帧实际画面，完全相同。损失为 0 意味着这一帧被预测出来了。",
    ariaEmbed: "嵌入损失随训练降到零。揭示后：两种状态损失同为零，一种里描述分成左转与右转两群，另一种里所有描述都挤在同一点上。",
    ariaHidden: "一条随训练降到零的损失曲线。尚未揭示。",
    ariaProbed: "一条探针线把第一团云分成两群，在塌缩的那一团里什么也分不开。",
  },
};
