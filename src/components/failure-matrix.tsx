...
export function FailureMatrix() {
  const t = useT();
  /**
   * Five risk names will not sit side by side across a phone. Below the
   * threshold the column headings turn on their side, which keeps every name
   * whole and leaves the row of bars its width.
   */
  ...
  return (
    <div>
      <div ref={ref} className="px-4 pt-6 md:px-8">
        <div>
          <div className={`grid gap-px bg-rule ${compact ? "grid-cols-[minmax(0,1fr)_repeat(5,1.5rem)]" : "grid-cols-[10rem_repeat(5,minmax(6rem,1fr))]"}`}>
            <div className={`bg-paper-raised ${compact ? "p-2" : "p-3"}`} />
            {RISKS.map((risk) => (
              <div key={risk} className={`bg-paper-raised ${compact ? "flex items-end px-1 py-2" : "p-3"}`}>
                <span className={`label !text-[0.58rem] ${compact ? "block whitespace-nowrap [writing-mode:vertical-rl]" : ""}`}>{t(`fm.risk.${risk}`)}</span>
              </div>
            ))}
            {CONTRACTS.map((contract) => (
              <MatrixRow key={contract} contract={contract} active={selected === contract} onSelect={() => setSelected(contract)} levels={MATRIX[contract]} t={t} compact={compact} />
            ))}
          </div>
        </div>
      </div>
...
function MatrixRow({ contract, active, onSelect, levels, t, compact }: { contract: string; active: boolean; onSelect: () => void; levels: Level[]; t: (key: string, vars?: Record<string, string | number>) => string; compact: boolean }) {
  return (
    <>
      <button onClick={onSelect} className={`text-left transition-colors ${compact ? "p-2" : "p-3"} ${active ? "bg-ink" : "bg-paper hover:bg-paper-raised"}`}>
        <span className={`label !text-[0.62rem] ${compact ? "break-words" : ""} ${active ? "!text-paper" : ""}`}>{t(`fm.contract.${contract}`)}</span>
      </button>
      {levels.map((level, index) => (
        <button key={index} onClick={onSelect} aria-label={`${t(`fm.contract.${contract}`)}: ${t(`fm.level.${level}`)}`}
          className={`flex items-center justify-center transition-colors ${compact ? "px-0.5 py-3" : "p-3"} ${active ? "bg-paper-raised" : "bg-paper"}`}>
