...
const TEXT: Record<string, Strings> = {
  ...
  zh: {
    ...
    rOf: "{a} 个，共 {b} 个",
    rLabels: "给出的按键标签",
    v0: "十二个变化，没有标签。模型得把每一个归到某个编码下。",
    v1: "四个编码覆盖了所有变化。没人告诉它哪个编码是哪个，而这几堆的行为像一个手柄。",
    v2: "按一个编码，模型就移动角色。这就是它找到的手柄。",
    aria0: "十二对来自小型平台游戏的前后帧，尚未分类，旁边是编号一到八的八个空箱。没有按键标签。",
    aria1: "十二对帧已分到八个箱中的四个：所有左移在一个箱里，所有右移在另一个，所有跳跃在第三个，原地不动的在第四个。",
    aria2: "按下了编码 {n}。在一张新的帧上，模型画出了{k}之后的角色。",
    ariaEmpty: "按下了编码 {n}。它从未用到，所以模型画出没有变化。",
    kinds: { left: "每一次左移", right: "每一次右移", jump: "每一次跳跃", still: "原地不动" },
    kindsShort: { left: "左移", right: "右移", jump: "跳跃", still: "不动" },
  },
};
