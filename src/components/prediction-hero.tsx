...
export function PredictionHero() {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      if (trail.length > 1) {
        ...
        for (let i = 1; i < trail.length; i++) {
          ctx!.globalAlpha = 0.22 + (i / trail.length) * 0.6;
          ctx!.beginPath();
    ...
    function loop() {
      ...
      if (frame % ROLLOUT_EVERY === 0) ghosts.push({ pts: imagine(), age: 0 });
    ...
    if (reduced) {
      // one static frame: the idea without the motion
      pos.x = W * 0.28;
      pos.y = H * 0.08;
      vel.x = BASE_VX;
      vel.y = 0;
      trail = [];
      ghosts = [{ pts: imagine(), age: 0 }];
      for (let i = 0; i < 110; i++) {
        step(pos, vel, TRUE_G, TRUE_REST, TRUE_DRAG);
        trail.push({ ...pos });
      }
