      ...
      <section className="mx-auto mt-28 max-w-[84rem] px-6 md:px-10">
        <div className="grid gap-x-16 gap-y-10 md:grid-cols-[13rem_minmax(0,40rem)]">
          ...
          <div className="prose">
            ...
            <p>
              第五拨人根本不是在描述一个系统。当人们争论一个语言模型「有没有世界模型」时，他们讲的是在一个为别的任务训练出来的网络内部发现的结构。支持这个断言的证据来自可解释性研究，而不是任何你能运行的东西。两个人可以在每一个事实上都一致，却依然争执不下，因为一个问的是系统能不能模拟，另一个问的是网络内部有没有某种东西。
            </p>
            <p>
              命名的混乱底下是一个真实而古老的题目。它从卡尔曼滤波器一路延伸到 Schmidhuber 1990 年的论文，再到 Dreamer、JEPA、Genie 和 Marble。相关文献很多，而几乎没有一篇是写给初来乍到的人看的。
            </p>
            <p>
              这本读物有九章，每一章都围绕一个你可以亲手拨弄的东西。唯一的前置要求是熟悉梯度和一点线性代数。第 1 章是那份指南。其余八章讲机制，并且每一章在重要的地方都会说明自己用的是哪一种定义。
            </p>
          </div>
        </div>
      </section>
